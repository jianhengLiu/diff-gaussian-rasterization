...
RasterizeGaussiansCUDA(
    ...
    const float tan_fovx, const float tan_fovy, const float center_x,
    const float center_y, const int image_height, const int image_width,
  ...
  if (P != 0) {
    ...
    rendered = CudaRasterizer::Rasterizer::forward(
        ...
        campos.contiguous().data<float>(), tan_fovx, tan_fovy, center_x,
        center_y, prefiltered, out_color.contiguous().data<float>(),
...
RasterizeGaussiansBackwardCUDA(
  ...
  torch::Tensor dL_dzdepths = torch::zeros({P, 1}, means3D.options());
  ...
  if (P != 0) {
    CudaRasterizer::Rasterizer::backward(
        ...
        dL_dzdepths.contiguous().data<float>(),
