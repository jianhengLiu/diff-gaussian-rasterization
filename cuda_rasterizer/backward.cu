...
__global__ void computeCov2DCUDA(int P, const float3 *means, const int *radii,
                                 const float *cov3Ds, const float f_x,
                                 float f_y, const float tan_fovx,
                                 float tan_fovy, const float *view_matrix,
                                 const float *dL_dconics,
                                 const float *dL_dzdepths, float3 *dL_dmeans,
                                 float *dL_dcov) {
  ...
  // glm is column major
  glm::mat3 J = glm::mat3(f_x / t.z, 0.0f, -(f_x * t.x) / (t.z * t.z), 0.0f,
                          f_y / t.z, -(f_y * t.y) / (t.z * t.z), 0, 0, 0);
  ...
  if (denom2inv != 0) {
    ...
    // TODO: check if 2* is not needed, as i calculate, delete it if it will
    // improve?
  ...
  float dL_dtz = -f_x * tz2 * dL_dJ00 - f_y * tz2 * dL_dJ11 +
                 (2 * f_x * t.x) * tz3 * dL_dJ02 +
                 (2 * f_y * t.y) * tz3 * dL_dJ12;
...
__global__ void __launch_bounds__(BLOCK_X *BLOCK_Y) renderCUDA(
    ...
    const float *__restrict__ zdepths, const float *__restrict__ final_Ts,
    const uint32_t *__restrict__ n_contrib,
    const float *__restrict__ dL_dpixels, const float *__restrict__ dL_ddepths,
  ...
  for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE) {
    ...
    if (range.x + progress < range.y) {
      ...
      for (int i = 0; i < C; i++)
        collected_colors[i * BLOCK_SIZE + block.thread_rank()] =
            colors[coll_id * C + i];
      collected_depths[block.thread_rank()] = zdepths[coll_id];
    }
    block.sync();
    ...
    for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++) {
      ...
      const float ddepth_dzdepth = alpha * T;
      ...
      last_depth = z;
      dL_dalpha += (z - accum_depth_rec) * dL_ddepth;
      dL_dalpha *= T;
...
void BACKWARD::preprocess(
    ...
    const float *dL_dzdepths, glm::vec3 *dL_dmean3D, float *dL_dcolor,
  ...
  computeCov2DCUDA<<<(P + 255) / 256, 256>>>(
      ...
      viewmatrix, dL_dconic, dL_dzdepths, (float3 *)dL_dmean3D, dL_dcov3D);
...
void BACKWARD::render(const dim3 grid, const dim3 block, const uint2 *ranges,
                      ...
                      const float *zdepths, const float *final_Ts,
                      const uint32_t *n_contrib, const float *dL_dpixels,
                      const float *dL_ddepths, float3 *dL_dmean2D,
                      float4 *dL_dconic2D, float *dL_dopacity,
                      float *dL_dcolors, float *dL_dzdepths) {
  renderCUDA<NUM_CHANNELS><<<grid, block>>>(
      ranges, point_list, W, H, bg_color, means2D, conic_opacity, colors,
      zdepths, final_Ts, n_contrib, dL_dpixels, dL_ddepths, dL_dmean2D,
      dL_dconic2D, dL_dopacity, dL_dcolors, dL_dzdepths);
}