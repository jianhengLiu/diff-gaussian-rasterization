...
void render(const dim3 grid, dim3 block, const uint2 *ranges,
            ...
            const float *colors, const float *zdepths, const float *final_Ts,
            const uint32_t *n_contrib, const float *dL_dpixels,
            const float *dL_ddepths, float3 *dL_dmean2D, float4 *dL_dconic2D,
            float *dL_dopacity, float *dL_dcolors, float *dL_dzdepths);
...
void preprocess(int P, int D, int M, const float3 *means, const int *radii,
                ...
                const float *dL_dzdepths, glm::vec3 *dL_dmeans,
