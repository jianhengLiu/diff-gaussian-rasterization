...
int CudaRasterizer::Rasterizer::forward(
    ...
    float tan_fovy, const float center_x, float center_y,
    const bool prefiltered, float *out_color, float *out_depth, int *radii,
    bool debug) {
  ...
  dim3 tile_grid((width + BLOCK_X - 1) / BLOCK_X,
                 (height + BLOCK_Y - 1) / BLOCK_Y, 1); // blocks
  ...
  CHECK_CUDA(FORWARD::preprocess(
                 ...
                 tan_fovx, tan_fovy, center_x, center_y, radii,
                 geomState.means2D, geomState.depths, geomState.cov3D,
  ...
  duplicateWithKeys<<<(P + 255) / 256, 256>>>(
      ...
      // The function takes an integer argument and returns the highest bit
      // position of the argument as an integer value. For example, if the
      // argument is 8 (binary 1000), the function returns 3, which is the
...
void CudaRasterizer::Rasterizer::backward(
    ...
    char *img_buffer, const float *dL_dpix, const float *dL_ddepth,
    float *dL_dmean2D, float *dL_dconic, float *dL_dopacity, float *dL_dcolor,
    float *dL_dzdepths, float *dL_dmean3D, float *dL_dcov3D, float *dL_dsh,
  ...
  const float *color_ptr =
      (colors_precomp != nullptr) ? colors_precomp : geomState.rgb;
  const float *zdepth_ptr = geomState.depths;
  CHECK_CUDA(BACKWARD::render(
                 tile_grid, block, imgState.ranges, binningState.point_list,
                 width, height, background, geomState.means2D,
                 geomState.conic_opacity, color_ptr, zdepth_ptr,
                 imgState.accum_alpha, imgState.n_contrib, dL_dpix, dL_ddepth,
                 (float3 *)dL_dmean2D, (float4 *)dL_dconic, dL_dopacity,
                 dL_dcolor, dL_dzdepths),
             debug)
  ...
  CHECK_CUDA(BACKWARD::preprocess(
                 ...
                 dL_dzdepths, (glm::vec3 *)dL_dmean3D, dL_dcolor, dL_dcov3D,
