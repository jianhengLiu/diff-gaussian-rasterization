...
preprocessCUDA(int P, int D, int M, const float *orig_points,
               ...
               float focal_y, const float center_x, float center_y, int *radii,
  ...
  float2 point_image = {ndc2Pix2(p_proj.x, W, center_x),
                        ndc2Pix2(p_proj.y, H, center_y)};
  ...
  depths[idx] = p_view.z;
  radii[idx] = my_radius;
  points_xy_image[idx] = point_image; // is it the image to show the initail sfm
                                      // point image?: it is 'means2D'
...
__global__ void __launch_bounds__(BLOCK_X *BLOCK_Y)
  ...
  uint2 pix = {pix_min.x + block.thread_index().x,
               pix_min.y + block.thread_index().y}; // threads
...
void FORWARD::preprocess(int P, int D, int M, const float *means3D,
                         ...
                         const float center_x, float center_y, int *radii,
  ...
  preprocessCUDA<NUM_CHANNELS><<<(P + 255) / 256, 256>>>(
      ...
      W, H, tan_fovx, tan_fovy, focal_x, focal_y, center_x, center_y, radii,
      means2D, depths, cov3Ds, rgb, conic_opacity, grid, tiles_touched,
      prefiltered);
}